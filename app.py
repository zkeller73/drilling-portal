...
page = st.sidebar.radio("Go to", ["Upload Report", "View Reports", "Estimates"])

# --- Estimates Page ---
if page == "Estimates":
    st.title("🧮 Enter Well Cost Estimates")
...
# --- View Reports Page ---
    ...
    if os.path.exists(REPORT_LOG):
        ...
        if not required_cols.issubset(df.columns):
            st.error("Some expected columns are missing in the report log.")
        else:
            ...
            if os.path.exists(ESTIMATE_FILE):
                ...
                st.info("No estimates found. Go to the Estimates page to enter them.")
            ...
            if "Filename" in df.columns and not df["Filename"].isna().all():
                ...
                st.info("No PDFs found in report log.")

            # Delete Report by Row
